...
import './SafeMath.sol';
import './SwitchV2Library.sol';

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library SwitchV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = SwitchV2Library.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, 'SwitchV2ArbitrageLibrary: ZERO_PAIR_RESERVES');
        ...
        if (aToB) {
            uint256 amountOut = SwitchV2Library.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = SwitchV2Library.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = SwitchV2Library.getReserves(factory, tokenA, tokenB);
        ISwitchV2Pair pair = ISwitchV2Pair(SwitchV2Library.pairFor(factory, tokenA, tokenB));
        bool feeOn = ISwitchV2Factory(factory).feeTo() != address(0);
